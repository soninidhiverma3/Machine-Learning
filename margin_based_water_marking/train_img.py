...
run = wandb.init(project="Watermark-ML-Project", name="cifar_10_base_Training")

def log_images_with_labels(images, labels, caption):
    # Convert to CPU and numpy for WandB compatibility
    images = images.cpu()
    labels = labels.cpu().numpy()
    # Format the images and labels for WandB
    wandb_images = [
        wandb.Image(image, caption=f"{caption} - Label: {label}")
...
def loop(model, query_model, loader, opt, lr_scheduler, epoch, logger, output_dir, max_epoch=100, train_type='standard', mode='train', device='cuda', addvar=None):
    ...
    if mode == 'test' and (epoch+1) % 20 == 0:
        ...
        log_images_with_labels(query, response, caption=f"Query Image at Epoch {epoch}")
    return meters
...
def train(args, output_dir):
    ...
    init_query, _ = query()
    query.train()
    grid_image = make_grid(init_query, nrow=10)

    # Log the grid image directly to WandB
    wandb.log({"query_image_init": wandb.Image(grid_image.permute(1, 2, 0).cpu().numpy(), caption="Initial Query Image")})
    ...
    wandb.log({"best_val_nat_acc": best_val_nat_acc, "best_val_query_acc": best_val_query_acc})
