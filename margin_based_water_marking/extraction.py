...
from loaders import  get_cifar10_loaders, get_svhn_loaders, get_cifar100_loaders
...
run = wandb.init(
    ...
    project="Watermark-ML-Project", name="SVHN-Extract_margin",
)
def extract_loop(model, teacher, query_model, loader, opt, lr_scheduler, epoch, logger, output_dir,
    ...
    for batch_idx, batch in enumerate(loader):
        ...
        if mode == 'train':
            wandb.log({'extract_loss': extract_loss.item(), 'epoch': epoch, 'batch_idx': batch_idx})
            meters.update({
                'extract loss': extract_loss.item()
            }, n=images.size(0))
        else:

                # Log all losses and accuracies to W&B
            wandb.log({
                    'nat_acc': nat_acc.item(),
                    'teach_acc': teacher_acc.item(),
                    'extract_loss': extract_loss.item(),
                    'epoch': epoch,
                    'batch_idx': batch_idx
                })
    ...
    with torch.no_grad():
        ...
        # Log query-related results to W&B
        wandb.log({
    ...
    wandb.log({"examples": [wandb.Image(img, caption=f"Image {i}") for i, img in enumerate(images[:5])]})
